                            ...
                            if (dtPostOrderParameter.Rows[0]["IncludeAddonService"].ToString() == Constant.FLAG_Y)
                            {
                                if (dtPostOrderParameter.Columns.Contains("AddonServices"))
                                {
                                    ...
                                    for (int i = 0; i < strservices.Length; i++)
                                    {
                                        ...
                                        if (strserviceSizetype[0] == Constant.PAINTING_SERIVCES)
                                        {
                                            DataTable dtPaintingservice = GetPaitingServiceRate(strserviceSizetype[0], strserviceSizetype[1], goods_type_flag, rate_type_flag, OrderDate, ref TotalPaintingCharge, ref TotalPaintingDiscount, ref Message);
                                        ...
                                        else if (strserviceSizetype[0] == Constant.CLEANING_SERIVCES)
                                        {
                                            DataTable dtCleaningService = GetCleaningServiceRate(strserviceSizetype[0], strserviceSizetype[1], goods_type_flag, rate_type_flag, OrderDate, ref TotalCleaningCharge, ref TotalCleaningDiscount, ref Message);
                                        ...
                                        else if (strserviceSizetype[0] == Constant.PESTCONTROL_SERIVCES)
                                        {
                                            DataTable dtPestControlservice = GetPestControlServiceRate(strserviceSizetype[0], strserviceSizetype[1], goods_type_flag, rate_type_flag, OrderDate, ref TotalPestControlCharge, ref TotalPestControlDiscount, ref Message);
                                                ...
                                                if (dtPostOrderParameter.Rows[0]["IncludeAddonService"].ToString() == Constant.FLAG_Y)
                                                {
                                                    if (dtPostOrderParameter.Columns.Contains("AddonServices"))
                                                    {
                                                        ...
                                                        for (int i = 0; i < strservices.Length; i++)
                                                        {
                                                            ...
                                                            if (strserviceSizetype[0] == Constant.CLEANING_SERIVCES)
                                                            {
                                                                DataTable dtCleaningService = GetCleaningServiceRate(strserviceSizetype[0], strserviceSizetype[1], goods_type_flag, rate_type_flag, OrderDate, ref TotalCleaningCharge, ref TotalCleaningDiscount, ref Message);
                                                                if (dtCleaningService == null)
                                                                    return null;
                                                            ...
                                                            if (strserviceSizetype[0] == Constant.PESTCONTROL_SERIVCES)
                                                            {
                                                                DataTable dtPestControlservice = GetPestControlServiceRate(strserviceSizetype[0], strserviceSizetype[1], goods_type_flag, rate_type_flag, OrderDate, ref TotalPestControlCharge, ref TotalPestControlDiscount, ref Message);
                                                                if (dtPestControlservice == null)
                                                                    return null;
                ...
                if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            command.Parameters.Clear();
                            StringBuilder SQLUpdate = new StringBuilder();
                            SQLUpdate.Append(@"UPDATE order_AddonService_details  
                                               SET    rem_amt_to_receive = @rem_amt_to_receive, modified_by = @modified_by, modified_date = @modified_date, modified_host = @modified_host, modified_device_id = @modified_device_id, modified_device_type = @modified_device_type ");
