...
namespace trukkerUAE.Controllers
{
    public class PaymentController : ServerBase
    {
        ...
        public string GetPaymentLink(string inqno)
        {
            ...
            //query1 = " select *  FROM  order_paymentdetails where load_inquiry_no=@inqno and  created_by='admin'  ";
        ...
        {
            String query = @" select order_paymentdetails.*,orders.* from order_paymentdetails 
                                left join orders on orders.load_inquiry_no=order_paymentdetails.load_inquiry_no  where Transaction_id= @TrnID ";
                        ...
                        if (dtOrder != null)
                        {
                            ...
                            if (dtOrder.Rows[0]["IncludeAddonService"].ToString() == Constant.FLAG_Y)
                            {
                                ...
                                if (dtaddonorder != null)
                                {
                                    ...
                                    //decimal addonChargewithdiscount = 0;
                                    //decimal Order_Current_rem_amt = Order_rem_amt_to_receive - Amount;
                                    //addonChargewithdiscount = Order_TotalAddServiceCharge;//- Order_TotalAddServiceDiscount;


                                    //decimal OrderBaseCharge = Order_Total_cost - addonChargewithdiscount;

                                    //if (Order_Current_rem_amt < addonChargewithdiscount)
                                    //{
                                    //    decimal New_rcv_amount = 0;
                                    //    decimal addon_rem_amt_to_receive = 0;

                                    //    decimal NewAddonRemAmt = 0;
                                    //    if ((Order_Total_cost - Order_rem_amt_to_receive) > OrderBaseCharge)
                                    //        NewAddonRemAmt = Amount;
                                    //    else
                                    //    {
                                    //        if (OrderBaseCharge < Amount)
                                    //            NewAddonRemAmt = Amount - OrderBaseCharge;

                                    //        if (OrderBaseCharge > Amount)
                                    //            NewAddonRemAmt = OrderBaseCharge - Amount;
                                    //    }

                                    //    for (int i = 0; i < dtaddonorder.Rows.Count; i++)
                                    //    {
                                    //        if (i == 0)
                                    //            New_rcv_amount = NewAddonRemAmt;
                    ...
                    if (dtOrder.Rows[0]["IncludeAddonService"].ToString() == Constant.FLAG_Y)
                    {
                        ...
                        if (dtaddonorder != null)
                        {
                            ...
                            if (dtaddonorder != null && dtaddonorder.Rows.Count > 0)
                            {
                                ...
                                //if (Order_Current_rem_amt < Order_TotalAddServiceCharge)
                                //{
                                //    decimal New_rcv_amount = 0;
                                //    decimal addon_rem_amt_to_receive = 0;

                                //    decimal NewAddonRemAmt = 0;
                                //    if ((Order_Total_cost - Order_rem_amt_to_receive) > Order_TotalCostWithoutAddon)
                                //        NewAddonRemAmt = Amount;
                                //    else
                                //    {
                                //        if (Order_TotalCostWithoutAddon < Amount)
                                //            NewAddonRemAmt = Amount - Order_TotalCostWithoutAddon;

                                //        if (Order_TotalCostWithoutAddon > Amount)
                                //            NewAddonRemAmt = Order_TotalCostWithoutAddon - Amount;
                                //    }
